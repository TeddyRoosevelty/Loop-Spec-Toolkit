...
Run the adversary:
- Once the surface and assets are mapped and the threats rated, determine the riskiest areas, then dispatch a `spec-doc-skills:threat-model-adversary` to work them.
...
- **Threat models folder** — `<plugin-data-path>/threat-models`.
- **Save File Name** — `<threat-models-folder>/<topic>-threat-model.md`.
